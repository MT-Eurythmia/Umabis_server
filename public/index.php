...
require __DIR__ . '/../src/server_auth.php';

/* Trying to understand how the following wrappers wrap each other might
 * lead to headache.
 */

function server_auth_wrapper($request, $action) {
	$server_name = $request->get('server_name');
	$server_token = $request->get('server_token');
	if (!$server_name || !$server_token)
		return '012';

	if ($e = ServerAuth\check_auth($server_name, $server_token))
		return sprintf('%03d', $e);
...
function command_wrapper($request, $action) {
	return server_auth_wrapper($request, function() use ($request, $action) {
...
function privileged_command_wrapper($request, $action) {
	return command_wrapper($request, function($name) use ($action) {
		// Check privileges
...
$app->path('api', function($request) use($app, $db) {
	$app->path('register', function($request) use($app, $db) {
		$app->post(function($request) use($db) {
			return server_auth_wrapper($request, function() use ($request, $db) {
	...
	$app->path('hello', function($request) use ($app, $db) {
		$app->post(function($request) use ($db) {
			...
			$password = $request->post('server_password');

			if (!$name || !$password)
				return '012';

			$token = ServerAuth\authenticate($name, $password);
			if (is_int($token))
				return sprintf('%03d', $token);
			...
			return '000' . json_encode(array(
				'SESSION_EXPIRATION_DELAY' => SESSION_EXPIRATION_DELAY,
				'SERVER_EXPIRATION_DELAY' => SERVER_EXPIRATION_DELAY,
				'VERSION' => VERSION,
				'NAME' => NAME,
	...
	$app->path('goodbye', function($request) use ($app, $db) {
		$app->post(function($request) use ($db) {
			return server_auth_wrapper($request, function() use ($request, $db) {
				$server_name = $request->post('server_name');

				return sprintf('%03d', ServerAuth\close_session($server_name));
			});
		});
	});
	...
	$app->path('server_ping', function($request) use ($app) {
		$app->post(function($request) {
			return server_auth_wrapper($request, function() {
				// Do nothing more
			});
		});
	});
